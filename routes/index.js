...
const { validateUsername} = require('../validations/validators')
...
router.post('/register', (req, res)=>{
    ...
    if(username.length < 1){
      return  res.status(400).send('Por favor, rellene el nombre de Usuario')
    }
    if(email.length < 1){
      return  res.status(400).send('Por favor, rellene el email')
    }
    if(password.length < 6){
       return res.status(400).send('La contraseña debe tener al menos 6 caractares')
    }
    ...
    .catch(error => res.status(404).send( 'El nombre de usuario ya existe'))
